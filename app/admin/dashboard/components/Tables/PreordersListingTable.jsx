...
const PreordersListingTable = ({ data, onOpenModal }) => {
  ...
  const DeleteItem = async(item) => {
    const {data,error}=await supabase.from('Preorder').delete().eq('id',item.id)
    if(error){
            ...
            {searchedData?.map((item, key) => (
                <tr
                  ...
                  {/* <td className="border-b dark:border-gray-500 border-[#eee] px-4 py-5 dark:border-strokedark">
                    ...
                    <h1 className="text-black dark:text-white text-sm line-through text-gray-500">{item.mrp}</h1>
                  </td> */}
                  <td className="border-b dark:border-gray-500 border-[#eee] px-4 py-5 dark:border-strokedark">
                    ...
                    {/* <td className="text-sm border-b dark:border-gray-500 border-[#eee] py-5 dark:border-strokedark">
                        <button className={`w-full justify-center flex gap-2 items-center ${
                            item.inStock
                                ? "bg-green-500"
                                : "bg-red-700"
                        
                        } text-white p-2 rounded-md`} onClick={()=>handleStock(item)}
                        >
                          {item.inStock?(
                              <><h1>In Stock</h1><Check size={20} /></>
                          ):(<><h1>Out of Stock</h1><X size={20} /></>)}
                        </button>
                    </td> */}
                </tr>
              ))}
...
export default PreordersListingTable;
